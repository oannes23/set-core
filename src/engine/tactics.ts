/* engine/tactics — the Tactics buttons. Move matches fill the meter (see ops.addTactics); at full it
   ...
   The three shape tactics mirror the card types — Attack / Defend / Move each flood the board toward
   their shape; Heat / Chill / Wild flood toward a colour. Ported from the prototype TACTICS. */
...
export const TACTICS: Record<string, Tactic> = {
  attack: { key: 'attack', name: 'Attack', run(s, _r, sink) { transmute(s, liveSlots(s, (c) => cardShape(c) !== SHAPE_ATTACK), { bias: { shape: SHAPE_ATTACK, shapeW: BIAS_W, mag: 2, magW: BIAS_W } }, sink) } },
  defend: { key: 'defend', name: 'Defend', run(s, _r, sink) { transmute(s, liveSlots(s, (c) => cardShape(c) !== SHAPE_DEFEND), { bias: { shape: SHAPE_DEFEND, shapeW: BIAS_W, mag: 2, magW: BIAS_W } }, sink) } },
  move: { key: 'move', name: 'Move', run(s, _r, sink) { transmute(s, liveSlots(s, (c) => cardShape(c) !== SHAPE_MOVE), { bias: { shape: SHAPE_MOVE, shapeW: BIAS_W, mag: 2, magW: BIAS_W } }, sink) } },
...
/** Spend the armed Tactics meter on a tactic: reshape the board toward its axis, then reset the meter.
 *  No-op unless armed. Returns whether it fired. */
